    ...
    request('https://chainz.cryptoid.info/colx/api.dws?q=multiaddr&key=904de45cd265&active='+addresses[0], function(err, response, body) {
      ...
      */
      ...
      try {
        ...
        balance = Number(addressBalance.final_balance);
        received = Number(addressBalance.total_received);
        try {
          spent = Number(addressBalance.total_sent);
        }
        catch (e2) {}
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function finish(mempoolDeltas) {
    if (utxos) {
      ...
      request("https://chainz.cryptoid.info/colx/api.dws?q=unspent&key=904de45cd265&active=" + addresses[0],
        function(err, response, body) {
          if (err || body === "Error getting unspent outputs")
            return callback(self._wrapRPCError(err || body));
          try {
            ...
            console.log('unspent '+addresses[0]+' request: '+JSON.stringify(utxos));
            self.utxosCache.set(cacheKey, utxos);
            callback(null, updateWithMempool(utxos, mempoolDeltas));
          }
          catch (e) {
            callback(self._wrapRPCError(e));
          }
        });
...
Bitcoin.prototype.getBlock = function(blockArg, callback) {
  ...
  function queryBlock(err, blockhash) {
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, true,//nonsense: false,
           function(err, response) {
          if (err) {
            return done(self._wrapRPCError(err));
          }
          //nonsense: var blockObj = bitcore.Block.fromString(response.result);
          var result = response.result;
          var blockObj = {
            hash: blockhash,//result.hash,
            version: result.version,
          ...
          //Can also set this (same data)
          self.blockOverviewCache.set(blockhash, blockObj);
          //console.log("getBlock "+blockhash+": height="+blockObj.height+", confirmations="+blockObj.confirmations+", size="+blockObj.size+", time="+blockObj.time);
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        // Fix broken height by grabbing it from the block
        if (!tx.height || tx.height <= 0) {
          client.getBlock(tx.blockHash, true, function(err2, response) {
            if (err2) {
              return done(self._wrapRPCError(err));
            }
            tx.height = response.height;
            console.log("raw tx with height fix: hex="+tx.hex+", blockHash="+tx.blockHash+", height="+tx.height);
            done(null, tx);
          });
        } else {
          console.log("raw tx without height fix: hex="+tx.hex+", blockHash="+tx.blockHash+", height="+tx.height);
          done(null, tx);
        }
      });
    }, callback);
