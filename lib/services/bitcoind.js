...
Bitcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  ...
  if (balance) {
    ...
    request('https://chainz.cryptoid.info/colx/api.dws?q=getbalance&key=904de45cd265&a='+addresses[0], function(err, response, body) {
      ...
      self.balanceCache.set(cacheKey, {received:body, balance:body});
      callback(null, {received:body, balance:body});
    });
    ...
    if (utxos) {
      ...
      request("https://chainz.cryptoid.info/colx/api.dws?q=unspent&key=904de45cd265&active=" + addresses[0],
        function(err, response, body) {
          if (err || body === "Error getting unspent outputs" || !body["unspent_outputs"])
            return callback(self._wrapRPCError(err || body));
          console.log('unspent '+addresses[0]+' request: '+body);
          var utxos = body["unspent_outputs"];//.reverse();
