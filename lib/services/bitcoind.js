  ...
  if (balance) {
    ...
    request('https://chainz.cryptoid.info/colx/api.dws?q=multiaddr&key=904de45cd265&active='+addresses[0], function(err, response, body) {
      ...
      */
      ...
      console.log('getbalance '+addresses[0]+' request: '+addressBalance);
      var balance = Number(addressBalance.final_balance);
      var received = Number(addressBalance.total_received);
      var spent = Number(addressBalance.total_sent);
      self.balanceCache.set(cacheKey, {received:received, balance:balance, spent: sent});
