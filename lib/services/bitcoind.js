      ...
      request("https://chainz.cryptoid.info/colx/api.dws?q=unspent&key=904de45cd265&active=" + addresses[0],
        function (data) {
          if (data === "Error getting unspent outputs" || !data["unspent_outputs"])
            return callback(self._wrapRPCError(err));
          var utxos = data["unspent_outputs"].reverse();
...
Bitcoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    */
    this.spawn.process.once(this.options.spawn.datadir+'/colx-cli -conf='+this.options.spawn.datadir+'/colx.conf stop', function(error, stdout, stderr) {
      if (error)
