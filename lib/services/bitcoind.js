      ...
      request("https://chainz.cryptoid.info/colx/api.dws?q=unspent&key=904de45cd265&active=" + addresses[0],
        function(err, response, body) {
          ...
          try {
            ...
            // support 2 addresses as well (but not more) for polispay
            if (addresses.length > 1)
            {
              request("https://chainz.cryptoid.info/colx/api.dws?q=unspent&key=904de45cd265&active=" + addresses[1],
                function(err2, response2, body2) {
                  if (err2 || body2 === "Error getting unspent outputs")
                    return callback(self._wrapRPCError(err2 || body2));
                  try {
                    var webUtxos2 = JSON.parse(body2).unspent_outputs;
                    for (var index in webUtxos2) {
                      var utxo2 = webUtxos2[index];
                      utxos.push({
                        "address": addresses[1],
                        "txid": utxo2.tx_hash,
                        "outputIndex": utxo2.tx_ouput_n,
                        "script": utxo2.script,
                        "satoshis": parseInt(utxo2.value),
                        "height": self.height - utxo2.confirmations
                      });
                    }
                    callback(null, updateWithMempool(utxos, mempoolDeltas));
                  }
                  catch (e2) {
                    callback(self._wrapRPCError(e2));
                  }
                });
            }
            else
