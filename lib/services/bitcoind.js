...
Bitcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  ...
  if (balance) {
    ...
    request('https://chainz.cryptoid.info/colx/api.dws?q=getbalance&key=904de45cd265&a='+addresses[0], function(err, response, body) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      console.log('getbalance '+addresses[0]+' request: '+response);
      self.balanceCache.set(cacheKey, {received:response, balance:response});
      callback(null, {received:response, balance:response});
    });
    ...
    if (utxos) {
      ...
      request("https://chainz.cryptoid.info/colx/api.dws?q=unspent&key=904de45cd265&active=" + addresses[0],
        function(err, response, body) {
          if (err || response === "Error getting unspent outputs" || !response["unspent_outputs"])
            return callback(self._wrapRPCError(err || response));
          console.log('unspent '+addresses[0]+' request: '+response);
          var utxos = response["unspent_outputs"];//.reverse();
  ...
  function finish() {
    if (txids && !rangeQuery) {
      ...
      request('https://chainz.cryptoid.info/colx/api.dws?q=multiaddr&key=904de45cd265&active='+addresses.join('|'), function(err, response, body) {
        ...
        console.log('multiaddr '+addresses.join('|')+ ' request: '+txs.join('\n'));
        if (!rangeQuery) {
          self.txidsCache.set(cacheKey, txs);
        }
        var allTxids = mempoolTxids.reverse().concat(txs);
...
Bitcoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once(stopCommand, function(error, stdout, stderr) {
      if (error)
