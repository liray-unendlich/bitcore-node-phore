...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and bitcore-node-colx.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/bitcore-node-colx/blob/bitcoind/docs/upgrade.md \n');
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/colxd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed bitcore-node-colx.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bitcore-node-colx.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in bitcore-node-colx services directory
